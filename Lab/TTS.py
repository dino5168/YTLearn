...
class TTSItem:
    def __init__(self, en_text=None, zh_text=None):
        self.en = en_text
        self.zh = zh_text

    def __str__(self):
        return f"EN: {self.en} | ZH: {self.zh}"
    ...
    def load_from_file(self, filename):
        ...
        with open(filename, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if lang == "en":
                    current = TTSItem(en_text=text)
                    self.items.append(current)
                elif lang == "zh" and current:
                    current.zh = text
...
async def main():
    reader = TTSReader()
    reader.load_from_file("Lab.txt")  # 你的檔案路徑
