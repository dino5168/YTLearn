...
def convert_srt_to_ass(srt_path: Path, output_path: Path):
    ...
    try:
        ...
        style.fontsize = 28
        style.primarycolor = Color(255, 255, 255)  # 白色字
        style.secondarycolor = Color(255, 255, 255)  # 白色字
        style.outlinecolor = Color(0, 0, 0)  # 黑色外框
        style.backcolor = Color(0, 0, 0)  # 黑色背景
        style.bold = False
        style.outline = 2  # 外框寬度
        style.shadow = 1  # 陰影
        style.alignment = 2  # 中下對齊
...
def generate_video(
    mp3_file: Path, ass_path: Path, output_dir: Path, bg_image: Optional[Path]
):
    """生成影片"""
